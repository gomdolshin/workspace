...
> 상세설명
  - 버블
    * 시간복잡도는 O(n^2)로 다른 정렬알고리즘보다 속도가 느리며 loop를 돌면서 인접 데이터간의 swap연산하는 정렬
  - 선택
    * 시간복잡도는 O(n^2), 구현방법 복잡, 대상의 최소나 최대 데이터를 선택, 잘 쓰이지 않음
  - 삽입
    * 시간복잡도는 O(n^2), 구현방법 쉬움, 이미 정렬된 데이터 범위에 정렬되지 않은 데이터(선택데이터)를 적절한 위치에 삽입
    * 적절한 위치를 탐색하는것이 관건인데 이때 **BinarySearch**(이진탐색)등과 같은 탐색알고리즘을 사용해주면 시간을 줄일수 있음
  - 퀵
    * 시간복잡도는 O(nlogn), 기준값(pivot)을 기준으로 크고작은 데이터를 분류하는것을 반복하여 정렬
    * 드디어 시간복잡도가 다른게 나옴, 하지만 pivot이 시간복잡도에 영향을 미침.
